...
import { extractStorageForPage, handleFrameNavigated } from "./context.js";
...
function extractionPage(requestData: (params: Record<string, unknown>) => unknown) {
  const client = {
...
describe("IndexedDB session context fidelity", () => {
  ...
  it("exports only JSON by-value RemoteObjects and drops opaque or unserializable records", async () => {
    ...
    const context = await extractStorageForPage(page, logger);
    const records = context.indexedDB?.["fixture.test"]?.[0]?.data[0]?.records;
